const animateCircle = t=>{
    ...
    let d = s.width / 2
      ...
      , g = 0;
    const m = t=>{
        (c ? (g = Math.floor(t / l * 360),
        Math.floor(t / l * 100)) : (g += o) / 360 * 100) <= e ? (ctx.clearRect(0, 0, s.width, s.height),
        ctx.beginPath(),
        ctx.lineWidth = h,
        ...
        ctx.stroke(),
        ctx.closePath(),
        c || requestAnimationFrame(m)) : c || cancelAnimationFrame(m)
    }
    ;
    c || requestAnimationFrame(m),
    c && (m(window.pageYOffset),
